...
<h1>문의게시판</h1>
	...
	<table border="1">
		...
		<tr>
			<td colspan="4"> <img src=" qna/img/${p.p_img}"> <br>${p.p_text} </td>
...
<c:if test="${fn:trim(p.p_user) eq loginName || loginName eq 'admin'}">
		<tr>
			<td colspan="3"></td>
			<td>
				<button onclick="location.href='QnaModifyController?no=${p.p_no}'">수정</button>	<button onclick="delCheck(${p.p_no})">삭제</button>
			</td>
		</tr>
		</c:if>
	  ...
	  <c:forEach var="c" items="${comments}">
		...
		<tr>
			<td colspan="4">  이전글 <a href="QnaDetailC?no=${prevP.p_no}"> ${prevP.p_title} </a></td>
		</tr>
		<tr>
			<td colspan="4">  다음글 <a href="QnaDetailC?no=${nextP.p_no}"> ${nextP.p_title} </a></td>
		</tr>
	</table>
</body>
</html>